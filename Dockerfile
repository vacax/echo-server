...
FROM gradle:8.12-jdk21-jammy AS build
...
COPY --from=build /home/gradle/src/build/libs/*.jar /app/echo-server.jar
ENTRYPOINT ["java", "-Djava.security.egd=file:/dev/./urandom","-jar","/app/echo-server.jar"]